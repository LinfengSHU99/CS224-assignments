...
def naiveSoftmaxLossAndGradient(
    ...
    U = outsideVectors
    v_c = centerWordVec
    # y_hat = np.exp(np.transpose(U) * v_c) / np.sum(np.exp(np.transpose(U) * v_c))
    y_hat = np.exp(U @ v_c) / np.sum(np.exp(U @ v_c))
    y = np.zeros(outsideVectors.shape[0])
    y[outsideWordIdx] = 1

    gradCenterVec = np.transpose((y_hat - y)) @ U
    gradOutsideVecs = np.transpose(v_c.reshape(v_c.shape[0], 1) @ np.transpose((y_hat - y).reshape(y.shape[0], 1)))

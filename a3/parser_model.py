...
class ParserModel(nn.Module):
    ...
    def __init__(self, embeddings, n_features=36,
        ...
        w = nn.init.xavier_uniform_(torch.empty(self.embed_size * self.n_features, self.hidden_size))
        b1 = nn.init.uniform_(torch.empty(self.hidden_size))
        self.embed_to_hidden_weight = nn.Parameter(w)
        self.embed_to_hidden_bias = nn.Parameter(b1)
        self.dropout = nn.Dropout()
        u = nn.init.xavier_uniform_(torch.empty(self.hidden_size, self.n_classes))
        b2 = nn.init.uniform_(torch.empty(self.n_classes))
